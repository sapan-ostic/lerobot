...
"""
Run SmolVLA policy inference on SO101 robot.

This script demonstrates how to run SmolVLA (a vision-language-action model) on a
single-arm SO101 robot. It loads normalization statistics from a recorded dataset
and uses them to properly denormalize the model's actions.

SmolVLA is more flexible than PI0 and can handle variable DOF robots directly
without padding requirements.

Usage:
    python smolvla_so101_inference.py
...
# Task configuration
TASK_DESCRIPTION = "Pick up the pen and place it in the penholder"
...
def load_dataset_stats(dataset_repo_id: str) -> dict:
    """
    Load dataset statistics for SmolVLA normalization.

    SmolVLA handles variable DOF robots directly, so no padding is needed
    unlike PI0 which expects 32 dimensions.
    ...
    print(
        f"  Action statistics (6 DOF): "
        f"mean={action_mean[:3].round(2)}, std={action_std[:3].round(2)}"
    )
...
def setup_robot() -> SO101Follower:
    ...
    # Configure cameras - SmolVLA expects specific camera names
    # Using camera1 and camera2 naming convention
    camera_config = {
...
def setup_policy(dataset_stats: dict, device: str):
    """
    Load SmolVLA model and create pre/post processors with SO101 statistics.
    ...
    preprocess, postprocess = make_pre_post_processors(
        ...
        preprocessor_overrides={
            "device_processor": {"device": str(device_obj)},
            "normalizer_processor": {"stats": dataset_stats},
        },
        postprocessor_overrides={
            "unnormalizer_processor": {"stats": dataset_stats},
        },
    )
...
def main():
    ...
    print("="*60)
    print("SmolVLA Inference on SO101 Robot")
    print("="*60)
